...
const SECRET_KEY = process.env.VARIANTPAY_SECRET_KEY; // AES key
...
// AES decryption
function decryptPayload({ payload, iv }) {
...
/**
 * 1️⃣ Create VariantPay Transaction → Return paymentLink to frontend
 */
router.post("/create-payment", async (req, res) => {
  try {
    const { amount, userId, name, email, mobile } = req.body;

    const reference_id = "TXN" + Date.now();
    ...
    const payloadData = {
      ...
      transfer_type: "1",
      purpose_message: "Skeylet Subscription Payment",

      // NO CARD FIELDS — because we use Hosted Payment Page
      card_number: "",
      card_expiry_month: "",
      card_expiry_year: "",
      card_cvv: "",
    ...
    if (decrypted.status !== "SUCCESS") {
      return res.status(400).json({ success: false, message: decrypted.message });
    }
    ...
    console.error("VariantPay create-payment error:", err.response?.data || err);
    res.status(500).json({ success: false, message: "Payment initiation failed" });
  }
});

/**
 * 2️⃣ CALLBACK URL (SET THIS IN VARIANTPAY DASHBOARD)
 */
router.post("/callback", async (req, res) => {
  try {
    console.log("VariantPay CALLBACK:", req.body);

    const data = req.body;

    if (data.status === "SUCCESS") {
      console.log("Payment success for:", data.cTxnId);
      // Save payment success to DB here
    } else {
      console.log("Payment failed:", data.cTxnId);
      // Save failed status
    }
