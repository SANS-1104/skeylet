...
export default function StudioSuperAdminSidebar({ onSelect, activeTab }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static top-0 left-0 h-screen w-64 bg-white backdrop-blur-lg border-r text-gray-700 flex flex-col justify-between items-center p-6 transform transition-transform duration-300 z-50
    ...
    ${isActive
        ...
        <div className="lower-sidebar">
          <button
            onClick={logout}
            className="w-full  flex items-center gap-3 px-4 py-3 rounded-xl text-red-600 hover:bg-red-50 transition outline-black border-2 hover:border-transparent"
          >
            <LogOut size={20} />
